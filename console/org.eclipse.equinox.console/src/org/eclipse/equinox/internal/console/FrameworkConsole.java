...
public class FrameworkConsole implements Runnable {
	...
	// do we use system properties ?
	// -Dosgi.service.console.port
	public FrameworkConsole(BundleContext context, int port) {
		this.context = context;
		this.args = null; // TODO how do we figure out args?
		this.useSocketStream = port > 0;
		this.port = port;

		String fileEncoding = context.getProperty("file.encoding") == null ? defaultEncoding : context.getProperty("file.encoding"); //$NON-NLS-1$ //$NON-NLS-2$
		String consoleEncoding = context.getProperty("osgi.console.encoding"); //$NON-NLS-1$
		this.encoding = consoleEncoding == null ? fileEncoding : consoleEncoding;
		this.blockOnready = context.getProperty("osgi.dev") != null || context.getProperty("osgi.console.blockOnReady") != null; //$NON-NLS-1$ //$NON-NLS-2$
		...
		this.osgicp = new FrameworkCommandProvider(context).intialize();
	}
	...
	private void getSocketStream() {
		try {
			System.out.println(NLS.bind(Messages.CONSOLE_LISTENING_ON_PORT, String.valueOf(port)));
			synchronized (this) {
				if (scsg == null)
	...
	protected void console() throws IOException {
		...
		String consolePrompt = "\r\n" + Messages.CONSOLE_PROMPT; //$NON-NLS-1$
		while (!disconnected()) {
			...
			while (!shutdown) {
				try {
					...
					if (!acceptConnections) {
						...
						o.println(Messages.CONSOLE_TELNET_CONNECTION_REFUSED);
						o.println(Messages.CONSOLE_TELNET_CURRENTLY_USED);
						o.println(Messages.CONSOLE_TELNET_ONE_CLIENT_ONLY);
						o.close();
						socket.close();
					} else {
						synchronized (lock) {
