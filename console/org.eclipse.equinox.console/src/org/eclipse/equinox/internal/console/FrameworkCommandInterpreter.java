...
public class FrameworkCommandInterpreter implements CommandInterpreter {
	...
	private void setMaximumLinesToScroll(int lines) {
		if (lines < 0) {
			throw new IllegalArgumentException(Messages.CONSOLE_LINES_TO_SCROLL_NEGATIVE_ERROR);
		}
			...
			if (Modifier.isPublic(method.getModifiers()) && method.getName().startsWith("get") && throwable.isAssignableFrom(method.getReturnType()) && (method.getParameterTypes().length == 0)) { //$NON-NLS-1$
				try {
					...
					if ((nested != null) && (nested != t)) {
						out.println(Messages.CONSOLE_NESTED_EXCEPTION);
	...
	public void printDictionary(Dictionary dic, String title) {
		...
		// TODO should we replace this somehow?
		Util.sort(keys);
	...
	public void printBundleResource(Bundle bundle, String resource) {
		...
		if (entry != null) {
			try {
				...
				System.err.println(NLS.bind(Messages.CONSOLE_ERROR_READING_RESOURCE, resource));
			}
		} else {
			println(NLS.bind(Messages.CONSOLE_RESOURCE_NOT_IN_BUNDLE, resource, bundle.toString()));
		}
	}
	...
	private void check4More() {
		...
		if (max > 0) {
			if (currentLineCount >= max) {
				out.print(Messages.CONSOLE_MORE);
				out.flush();
	...
	public String getHelp() {
		...
		help.append(Messages.CONSOLE_HELP_CONTROLLING_CONSOLE_HEADING);
		help.append(newline);
		help.append(tab);
		help.append("more - "); //$NON-NLS-1$
		help.append(Messages.CONSOLE_HELP_MORE);
		if (con.getUseSocketStream()) {
			help.append(newline);
			help.append(tab);
			help.append("disconnect - "); //$NON-NLS-1$
			help.append(Messages.CONSOLE_HELP_DISCONNECT);
		}
		help.append(newline);
	...
	public void _more() throws Exception {
		if (confirm(Messages.CONSOLE_CONFIRM_MORE, true)) {
			int lines = prompt(newline + Messages.CONSOLE_MORE_ENTER_LINES, 24);
	...
	private void _disconnect() throws Exception {
		if (confirm(Messages.CONSOLE_CONFIRM_DISCONNECT, true)) {
			con.disconnect();
		}
	}
	...
	protected boolean confirm(String string, boolean defaultAnswer) {
		synchronized (out) {
			if (string.length() > 0) {
				...
				print(Messages.CONSOLE_CONFIRM);
			}
			print(" (" + Messages.CONSOLE_CONFIRM_VALUES); //$NON-NLS-1$
			if (defaultAnswer) {
				print(Messages.CONSOLE_Y + ") "); //$NON-NLS-1$
			} else {
				print(Messages.CONSOLE_N + ") "); //$NON-NLS-1$
			}
		}
		...
		return input.toLowerCase().charAt(0) == Messages.CONSOLE_Y.charAt(0);
	}
	...
	protected String prompt(String string, String defaultAnswer) {
		if (string.length() > 0) {
			if (defaultAnswer.length() > 0) {
				...
				buf.append(Messages.CONSOLE_PROMPT_DEFAULT);
				buf.append("="); //$NON-NLS-1$
	...
	protected int prompt(String string, int defaultAnswer) {
		...
		for (int j = 0; j < 3; j++) {
			...
			println(Messages.CONSOLE_INVALID_INPUT);
		}
		println(Messages.CONSOLE_TOO_MUCH_INVALID_INPUT);
		return defaultAnswer;
	}
}
