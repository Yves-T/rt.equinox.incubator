...
public class CachingTest extends WeavingTestCase {
	...
	public final static boolean debug = Boolean.getBoolean("org.eclipse.osgi.aspectj.tests.CachingTest.debug");
...
//	private List installedBundles = new LinkedList();
	...
	public void testCachingService () {
		ServiceReference ref = context.getServiceReference(ICachingService.class.getName());
		assertNotNull("Cannot find caching service",ref);
		ICachingService singletonCachingService = (ICachingService)context.getService(ref);
		assertNotNull("Cannot instantiate caching service",singletonCachingService);
	}
	...
	private Class runHello (String className) {
		...
		try {
			...
			if (debug) System.out.println("? CachingTest.testHello() clazz=" + clazz + "@" + Integer.toString(clazz.hashCode(),16) + ", loader=" + clazz.getClassLoader());
...
//		System.out.println("? CachingTest.testHelloAndTracing() host=" + Platform.getHosts(bundle3)[0].getSymbolicName());
		try {
			clazz = bundle1.loadClass(className);
			if (debug) System.out.println("? CachingTest.testHelloAndTracing() clazz=" + clazz + "@" + Integer.toString(clazz.hashCode(),16) + ", loader=" + clazz.getClassLoader());
			assertNotSame(previousClazz,clazz);
			previousClazz = clazz;
			runMain(clazz,new String[] {});
		}
...
//			installedBundles.remove(bundle);
	...
	protected void uninstallBundles (Bundle[] bundles) {
		if (debug) System.out.println("> CachingTest.uninstallBundles() bundles=" + Arrays.asList(bundles));
...
//			installedBundles.remove(bundle);
		...
		if (debug) System.out.println("< CachingTest.uninstallBundles() states=" + getBundleStates(bundles));
	}
